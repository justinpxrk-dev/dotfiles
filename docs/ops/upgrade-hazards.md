...
2. **Alias names are OS- and Stats-specific.** An alias name must equal its source's `"<owner>,<name>"`. On macOS 26 third-party items are owned by `Control Center` (e.g. `Control Center,CPU`), not `Stats` — a macOS major upgrade, a Stats update, or toggling a module's widget type / `oneView` can rename them. Re-discover with `sketchybar --query default_menu_items` and update `constants/item.lua`. The current map assumes CPU/GPU/Sensors enabled (single merged item), RAM with `oneView` off (separate `_state` + bar chart), and the **battery** with `oneView` off — which makes it two items, the glyph (`Battery_battery`) and a "mini" percentage (`Battery_mini`). Match those split battery names, **not** the merged `Control Center,Battery` (gone once `oneView` is off) nor Stats' bundle id `eu.exelban.Stats` (shared by hidden copies, resolves to a dimmed one). The macOS **clock** (`Control Center,Clock`, mirrored as the right-end clock pill) is a separate alias on these same couplings.
...
4. **Pill framing assumes frameless charts + a fixed recording-dot position.** The five Stats widgets (CPU/GPU/RAM/Sensors/battery) are framed in one pill (`STATS_PILL`, `plugins/resources.lua`), so Stats' own per-module chart frame must be **off** — otherwise it's a frame inside the pill border (frame-in-frame), and it shifts the image dimensions the paddings were measured against. That toggle lives in Stats' prefs, **not** these dotfiles, so it is not replayed on a fresh machine — disable it per module in Stats after install. The battery percentage's right inset (the Stats pill's right-most member) and the trailing `PILL_TRAIL` spacer are also tuned against the fixed screen position of the macOS screen-recording indicator dot (from coupling 1, it overlaps the bar's far-right corner by ~9px); the spacer is ~11px, so the visible gap is ~2px — if that dot moves on an OS change, the clearance drifts. Every per-element `ipad_l`/`ipad_r` is pixel-measured against Stats' baked image whitespace, so re-measure (screenshot + pixel-measure) if Stats' rendering changes.
...
## Sketchybar updates pill (Nerd Font glyphs + provider scopes)

The top bar's updates pill (`dot_config/sketchybar/plugins/updates.lua`, `event/handlers/updates.lua`, `event/providers/*.sh`) leans on a few things nothing checks:

1. **Symbols Nerd Font.** The segment glyphs render in `"Symbols Nerd Font"` (`constants/font.lua`) — the proportional standalone symbols font, installed **separately** from the MonoLisa Nerd Font patch, so a fresh machine without it shows tofu (□) for every glyph. The codepoints are pinned in `constants/icon.lua` as `\u{...}` escapes — `nf-dev-homebrew` (`U+E7FD`) and `nf-md-chef_hat` (`U+F0B7C`) — written as escapes because BMP private-use glyphs don't survive copy/paste through every tool. A Nerd Fonts **major version** can remap codepoints (v3 moved many Font Awesome glyphs, e.g. the beer mug to `U+F0FC`/`fa-beer_mug_empty`); if a glyph turns to tofu after a font upgrade, re-resolve the name against the installed font's cmap/post tables.
2. **mise scope is hardcoded.** `event/providers/mise.sh` runs `mise outdated -C "$HOME/.local/share/chezmoi"` — pinned to the dotfiles source dir because this machine has no global mise tools (an empty `~/.config/mise/config.toml`), so a global check would always read 0 and the segment would never appear. If the chezmoi source moves, or you want the segment to track a different project, edit `mise_dir`. That target's `mise.toml` must also be **trusted** (`mise trust`) or the count silently reads 0.
3. **brew needs SIGCHLD reset (the perl shim in `brew.sh`).** sketchybar runs item scripts with `SIGCHLD` _ignored_ (`SIG_IGN`), inherited across fork/exec. Under `SIG_IGN` a process can't reap its children, so Ruby (brew) can't read the exit status of anything it shells out to and aborts with "undefined method 'exitstatus'/'success?' for nil" — leaving empty stdout and a phantom 0 count (segment silently never appears). `brew.sh` therefore exec's brew via `perl -e '$SIG{CHLD}="DEFAULT"; exec @ARGV'` to restore the default disposition first. mise is immune (a static binary that doesn't depend on the inherited disposition). If a new provider shells out via a runtime with the same assumption (Ruby/Python/Node), it needs the same shim. This reproduces only under sketchybar's `exec`, never from a normal shell — so it won't show up testing the script by hand.
4. **Initial poll is explicit.** sketchybar does NOT fire a `routine` at startup, so `plugins/updates.lua` polls each provider once at load (in addition to the `update_freq` subscription); without that the pill stays empty until the first timer tick. A hidden segment keeps receiving `routine` (independent of `drawing`), so a count that dropped to 0 reappears later.
5. **Apple-Silicon absolute paths.** The scripts call `/opt/homebrew/bin/{brew,mise,jq}` and `/usr/bin/perl` directly (sketchybar's `exec` runs with a minimal PATH). On an Intel mac the Homebrew bins live under `/usr/local/bin`; update the scripts.
6. **The instant-refresh path couples two files.** It spans `plugins/updates.lua` (registers + subscribes to `updates_<provider>_refresh` and reads the pushed `COUNT`) and the zsh `brew`/`mise` wrapper functions in `dot_config/zsh/dot_zshrc_aliases` (which, after a successful upgrade, compute the count via the provider script and fire `sketchybar --trigger updates_<provider>_refresh COUNT=…`). The event name **and** the `COUNT` key are the contract — rename a provider or change either on one side and the other goes silently stale (the pill falls back to its periodic poll). The wrappers fire only in an interactive shell and only for the listed subcommands; a scripted/non-interactive upgrade won't push (the periodic poll backstops it).

**Failure mode:** glyphs show as tofu after a Nerd Font upgrade (codepoint remap) or on a machine missing Symbols Nerd Font; the brew segment silently never appears if the SIGCHLD shim is dropped; the mise segment never appears if its target dir is wrong or untrusted; both segments stay blank on an Intel mac until the paths are fixed; after an upgrade the pill lags up to an hour if the refresh-event names drift apart — none flagged at `chezmoi apply`.
