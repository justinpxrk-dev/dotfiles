...
My personal dotfiles, configurable for use on any of my machines.

## Installation

### Create `chezmoi` configuration file

Replace `PROFILE` with desired profile (personal, work).
...
```sh
chezmoi apply ~/.config/git/.gitconfig
```
...
> Some settings, such as those in location services, cannot be scripted. Manually configured settings are documented in [MacOS Manual System Settings.md](<docs/MacOS Manual System Settings.md>).
